...
import { Clock, GitBranch, GitCommit, TerminalSquare } from "lucide-react"
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/30 text-left">
              ...
              <th className="px-4 py-2.5 text-xs font-medium text-muted-foreground hidden sm:table-cell">Message</th>
              <th className="px-4 py-2.5 text-xs font-medium text-muted-foreground">
                <span className="hidden sm:inline">Branch</span>
                <span className="sm:hidden">Info</span>
              </th>
              <th className="px-4 py-2.5 text-xs font-medium text-muted-foreground hidden sm:table-cell">Commit</th>
              <th className="px-4 py-2.5 text-xs font-medium text-muted-foreground">Status</th>
              <th className="px-4 py-2.5 text-xs font-medium text-muted-foreground hidden sm:table-cell">Duration</th>
          ...
          <tbody>
            {deployments.map((d) => (
              <tr key={d.id} className="border-b last:border-0 hover:bg-muted/20 transition-colors">
                ...
                <td className="px-4 py-2.5 text-muted-foreground truncate max-w-[200px] hidden sm:table-cell">
                  {d.message}
                </td>
